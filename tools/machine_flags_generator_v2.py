...
class MachineFlagsGeneratorV2:
    ...
    def generate_with_abi(self, abi: str = None) -> Tuple[str, Dict[str, Any]]:
        ...
        selected = random.sample(list(all_flags.keys()), min(num_additional, len(all_flags)))
    ...
    def generate_multiple(self, count: int, vary_abi: bool = False) -> List[Dict[str, Any]]:
...
def main():
    ...
    parser.add_argument("--vary-abi", action="store_true", help="Vary ABI for more extension variation")
    ...
    for i, result in enumerate(results):
        ...
        config_data["llc_flags"] = gen.config_to_llc_flags(result["config"], result["abi"])
