...
if not json_file_path.exists():
    raise FileNotFoundError(f"Training JSON not found at: {json_file_path}.\n"
...
for entry in training_data:
    ...
    grouped_data[program]['sequences'].append(entry['pass_sequence'])  # Keep as list for nested structure
...
print(f"  - features_*: {len([c for c in df.columns if c.startswith('features_')])} feature columns")
