...
class TrainingDataGenerator:
    ...
    def compile_to_executable(self, bc_file: Path) -> Path:
        ...
        if self.target_arch in ['riscv64', 'riscv32']:
            ...
            try:
                subprocess.run(
                    ['llc', f'-march={march}', '-mattr=+m,+a,+f,+d,+c', 
                     str(bc_file), '-o', str(asm_file)],
                    check=True,
                    capture_output=True,
                    timeout=30
                )
    ...
    def save_dataset(self, dataset: Dict[str, Any], filename: str = "training_data.json"):
...
''')
            ...
            result = subprocess.run(
                ['opt', f'-passes={opt_map[opt_level]}', '--debug-pass-manager', bc_path, '-o', '/dev/null'],
                capture_output=True,
                text=True,
                timeout=5
            )
        ...
        for program in programs:
            ...
            for opt_level in ['-O0', '-O1', '-O2', '-O3']:
                try:
                    ...
                    print(f"Error generating baseline for {program.name} {opt_level}: {e}")
            
            baselines[program.stem] = program_baselines
        
        return baselines
    
    def generate_enhanced_baselines(self, programs: List[Path] = None, verbose: bool = True) -> Dict[str, Any]:
        """
...
def main():
    ...
    if args.enhanced_baselines:
        ...
        programs = generator.find_programs()
        enhanced_baselines = generator.generate_enhanced_baselines(programs, verbose=not args.quiet)
